...
def run_master(master_redis_cfg, log_dir, exp):
    ...
    episodes_target = exp['episodes_target'] if 'episodes_target' in exp else 0
    timesteps_target = exp['timesteps_target'] if 'timesteps_target' in exp else 0
    num_elites = exp['num_elites']
    population_score = np.array([])

    snapshot_path = exp['snapshot_path'] if 'snapshot_path' in exp else '.'
    snapshot_count = exp['snapshot_count'] if 'snapshot_count' in exp else 0
    snapshots = []

    while episodes_so_far < episodes_target or timesteps_so_far < timesteps_target:
        ...
        if config.snapshot_freq != 0:
            filename = os.path.join(
                snapshot_path,
                'snapshot_iter{:05d}_rew{}.h5'.format(
                    curr_task_id,
